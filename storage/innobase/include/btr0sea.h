...
struct btr_search_sys_t
{
  ...
  struct partition
  {
    ...
    char pad[(CPU_LEVEL1_DCACHE_LINESIZE - sizeof(frw_lock) -
  ...
  {
    ut_ad(!index.table->space || index.table->space->id == index.table->space_id);
    return get_part(ulint(index.id), index.table->space_id);
  }

  /** Get the search latch for the adaptive hash index partition */
  frw_lock *get_latch(const dict_index_t &index) const
...
inline ulint dict_index_t::n_ahi_pages() const
{
  if (!btr_search_enabled)
    return 0;
  frw_lock *latch= &btr_search_sys.get_part(*this)->latch;
  latch->rd_lock();
